...
Correct on a CRT, a bob in an emulator. The offset helper at `00105AC0` has three call sites; the one that fires is inside the present function, on the branch taken only while the mode global `002EE4CC` is 1 — a state the game enters around movie playback, its setter at `001E1AF0` having no callers outside the cluster at `002BB9F8`–`002BBFC0` that wraps the `MOVIE\NTSC\*.PSS` loads. Of the other two, one is gated on `002EE4A8`, which read 0 in every state sampled; the other sits in the vsync handler at `00164530` behind a flag that was not sampled, so treat it as unruled-out rather than dead. So when new shimmer is reported after a frame-rate patch, diff the two draw environments before blaming the emulator's deinterlacer; and note that a No-Interlacing group would settle both artefacts, since freezing `inter` also freezes the field flag `sceGsSyncV` feeds that helper.
