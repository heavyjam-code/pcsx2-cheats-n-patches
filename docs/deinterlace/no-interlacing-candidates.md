...
| 13 | Way of the Samurai 2 | `SLUS-20893` (×2 CRCs) | B, but not 1–3 instructions | Measured, not inferred: `640x448 -> 1280x896`, `SMODE2 = 1`, `DH = 448`, full-frame render. The one-word interlace flip applies cleanly and still blanks the output — see the correction below for the addresses and why. Neither `158FA006` (the undub build this repo already patches) nor `1B3EDC36` has a deinterlacing group; both ship only `[Widescreen 16:9]`. |
...
- **Mortal Kombat: Shaolin Monks** `SLUS-21087` — the existing code is explicitly incomplete, fixing 3D but not menus or HUD, and people are still asking. **Booted 2026-09-05 and the artefact did not reproduce**, pause menu included; see the correction below before budgeting anything.
...
**Verify with a five-minute boot test** (hold Triangle+X at the PS2 logo, and check the options screen) before disassembling any of these, whose status rests on absence from a 480p list rather than a positive statement: Just Cause, X-Men Legends 1 & 2, Splinter Cell: Double Agent, The Suffering, Sonic Heroes, Persona 4, Raiden III, SRW OG: Original Generations. Way of the Samurai 2 has since been measured off that list — see below.
...
**Way of the Samurai 2 leaves that tier confirmed interlaced, and confirmed not cheap.** Its display setup at `001E1BF0` reaches `sceGsResetGraph(mode=0, inter=1, omode=2, ffmd)` through the wrapper at `00131A80`, tail-calling `SetGsCrt` at `00103DA8`; live, the libgraph state at `002CD130` reads `inter=1, omode=2`, the draw envs scissor `640x448`, and the programmed display env is `SMODE2 = 1` (INT=1, FFMD=0) with `DH = 448`, `MAGV = 1`, `DW = 2560 / MAGH = 4`, `DY = 50`. Full 448-line frame, interlaced only on output — the TIER-B signature exactly. **The one-word flip still fails.** Forcing libgraph non-interlaced in the wrapper (`00131a90`, `move s4,a1` → `move s4,zero`) propagates through the whole chain on its own — SMODE2 `1 → 2`, `DY 50 → 25`, and the field offset stands down — and PCSX2 then renders nothing at all, because a 448-line `DISPLAY` does not fit the 240-line non-interlaced NTSC frame. The tell is that F8 at `ScreenshotSize = 2` returns a **1×1, 70-byte PNG**: that is an empty CRTC rect, not the `StartFullscreen` bug. Read a 1×1 as a failed mode change and go rework the `DH`/`DY` pair, or move `omode` to libgraph's DTV 480p branch (`00104118`, keyed on `omode == 3`, carrying the `0x50` constant at `00104120`).

**Samurai Western needs neither group, and it is the same Acquire engine that made Way of the Samurai 2 look expensive.** `SLUS-21187_9EE4D67B`, measured 2026-09-05. Same `MW MIPS C Compiler (2.4.1.01)` build and the same display module: `sceGsSetHalfOffset` at `0010DDD0`, `sceGsResetGraph` at `0010BFC0`, the `SetGsCrt` stub at `0011ABA0`, the libgraph state struct at `002AE230`, and a display wrapper at `0013EAA0` reached from the video-mode selector at `0014B270`. `$gp = 002D0870`; the context pointer is at `002C8F80`, its draw environments at `+0x80` and `+0x170`, the vsync interval at `+0x0c`.

Live at gameplay: `inter=1, omode=2, ffmd=0`, scissor `640x448`, `640x448 -> 1280x896` at 2x. **Both draw environments sit at `OFY = 29184` and never move** - 22895 samples over 3 s, zero transitions, while the field flag at `002C8F04` toggles 60 times a second beside them. The half-offset block is unreachable here: the present function tests the `ffmd` global at `0013E2AC` (`lw v0, -31044(gp)` / `bne v0, 1`) before it reaches the `slti at, v0, 2` interval gate, and `ffmd` is 0. That is the one structural difference from Way of the Samurai 2, where the same block is gated on a display-mode global that the title screen sets to 1.

**It is also already a 60 fps game** - vsync interval 1, `ctx+0x14` reads 1 field per presented frame in every sample, 60.0 vblanks/s, and the engine's FPS constant at `002C8880` is written 60 by the NTSC branch of the mode selector (`0014B36C`) against 50 by the PAL branch. There is no 30 to unlock.

Eight screen-resolution frames on an idle savestate, deinterlacing on Automatic: every frame scores best at a vertical shift of **0** against the first, and comb energy is flat at 1.01 across all eight. Nothing alternates. A `[No-Interlacing]` group would change no pixels.

The 480p route was built anyway and works, which is worth recording because it is the option the Way of the Samurai 2 entry above lists and never tested. This libgraph's `sceGsSetDefDispEnv` at `0010C278` dispatches on `omode` - 2 is NTSC (`0010C378`), 3 is PAL (`0010C42C`), and **anything else falls through to a real DTV480P branch at `0010C53C`** carrying the `0x50` left in `v0` by the delay slot at `0010C430`. Two words in the selector's NTSC branch - `0014B31C: 24050001 -> 24050000` (inter 0) and `0014B320: 24060002 -> 24060050` (omode `SCE_GS_DTV480P`) - read back correctly on a cold boot, leave the libgraph state at `inter=0, omode=80`, and make PCSX2 log `UpdateVSyncRate: Mode Changed to SDTV 480p`. It renders; it does not blank the way the one-word flip does on Way of the Samurai 2. It is still not shipped, but not for the reason first written here: with `pcrtc_offsets` on the 480p frame looks like it sits high and small next to stock, and that is **not** a `DY` error - interlaced `DH` counts half-lines and progressive `DH` counts whole lines, so the two are drawn at different raster scales and cannot be compared that way. The Dragon Quest VIII entry below settles it properly. It is unshipped because the game's output is already clean, so the group would buy nothing.


**Red Dead Revolver needs no group either, and it is the counter-example to the entry below.** `SLUS-20500_E169BAF8`, measured 2026-09-05 in Chapter 1 gameplay. First: **its shipped ELF cannot be scanned.** The boot ELF loads at `00800000` and contains not one `syscall`, not one `lui 0x1200` and no half-offset add; the game unpacks itself to `00100000` at run time, which is where the bundled pnach's addresses (`001018AC`, `00152D3C`) point. A signature scan of the file returns zero and means nothing. Work from a savestate's `eeMemory.bin` instead - every tool in this repo runs on a flat RAM image with a base of 0.

In RAM: `sceGsSetHalfOffset` at `003CA618`, its `SetGsCrt` stub at `003CAE20`, `sceGsResetGraph` tail-calling it at `003C96E0`. There is exactly **one** half-offset call site, `003BAA3C`, and it hands the helper the draw environments at `0079FEB0` and `0079FFA0`, the field flag at `00762084`, and a gate: `lbu v1, -8180(v0)` / `beqz` at `003BAA08`. Live, scissor is `512x448`, `512x448 -> 1024x896` at 2x, and **both draw environments read `OFY = 29184` with zero transitions** - 18109 samples, then 29858 more. Eight screen-resolution frames on an idle savestate at Automatic deinterlacing all score best at shift **0**, comb energy flat at 0.70.

**And the 60 fps group does not switch field rendering on here.** It is the same idiom: the shipped word `201018AC: 24030002 -> 24030001` is `li v1, 2 -> 1` feeding `sw v1, -21620(v0)`, a vsync interval at `004FAB8C` - and the game rewrites that global every frame, which is why the shipped line is `place=1` and why a PINE write to the *variable* reverts within a second while a write to the *instruction* sticks. Measured on the engine's own buffer counter at `0079FE40`: **30.0/s stock, 60.0/s with the instruction patched, 30.0/s restored**, and the draw environments stay at `29184 / 29184` throughout. The difference from Way of the Samurai 2 is structural, not luck: there the offset refresh sits behind `slti at, v0, 2` on the interval, so raising the frame rate is what opens it; here the only gate is a byte flag that is 0, with no interval test in front of it. So check the gate before assuming the trap - the idiom being identical does not make the consequence identical.


**Dragon Quest VIII has a complete dormant 480p mode, three data words away - and it is shipped, with a fourth word that took a day.** `SLUS-21207_F4715852` - greenfield, no bundled pnach on any serial or CRC. Same `MW MIPS C Compiler (2.4.1.01)` libgraph as the two above: `SetGsCrt` stub at `00116420`, `sceGsResetGraph` at `00108398`, libgraph state at `00390C50`; `$gp = 003D9770`. There is **no half-offset helper at all** - not one `64420008` in the image - so this engine has no per-field `XYOFFSET` mechanism to go wrong. Live it runs `inter=1, omode=2, ffmd=0` over a `512x448` buffer, `512x448 -> 1024x896` at 2x, and six screen-resolution frames at default settings all score best at shift **0** with comb energy flat at 1.32. Nothing to deinterlace, same as the other two.

The interesting part is the mode table. `set_video_mode` at `00146750` stores its index into `*(a0+0x260)` and dispatches through a **15-entry jump table at `003B5C50`**. Modes 2/3/4 are NTSC - they enter *after* an `li s2, 0x50` and so keep the `li s2, 2` default set at `00146770`. Modes 6/7/8 are the same three resolutions entering four bytes earlier, **on the `li s2, 0x50` - `SCE_GS_DTV480P`**; 9-14 are PAL. So each NTSC entry points exactly one instruction past its own 480p twin, and three data words move them: `003B5C58: 001467B0 -> 001467AC`, `003B5C5C: 001467C8 -> 001467C4`, `003B5C60: 001467E0 -> 001467DC`. The live config struct is at `003E4B00` and reads mode 2, `512x448`.

Cold booted with those three words, it works exactly as advertised: PCSX2 logs `Mode Changed to SDTV 480p`, libgraph reads `inter=0, omode=0x50, ffmd=1`, and the picture renders. **And the geometry is right** - forced to `AspectRatio = 4:3` the patched frame matches stock to the pixel, 2175x1630 with content at rows 241-1534 and columns 106-2037 in both. On the default `Auto 4:3/3:2` the patched frame is 2446 px wide instead of 2175, exactly the 720/640 raster ratio, because PCSX2's *auto* rule picks 3:2 for a progressive mode. That is a display-policy consequence every `480p Mode` group in the bundle shares, not a `DISPLAY` error - and it is the measurement that corrects the Way of the Samurai / Samurai Western note above.

**Then it goes black - and the first diagnosis written here was wrong.** Two cold boots driven by the identical key sequence: stock reaches a picture, the patched build sits at mean 0.0 for forty seconds with the log still reading SDTV 480p. The guess recorded at the time - that the game's four out-of-libgraph GS display writes reprogram the CRTC - did not survive reading the real privileged registers out of the savestate: every register was correct, the game's own `DISPLAY` decoded to a sane 480p rect, and NOPping the `DISPLAY1` write only made things worse. What was actually wrong: **the game had stopped.** VRAM's drawn-frame region was empty, presented frames read 0.0/s against 60 vblanks, and the EE `pc` from the savestate sat inside `sceGsSyncV`, called from a two-instruction loop at `001A0DC0` that repeats while the return value is 1 - a wait for an even field. `sceGsSyncV` returns the field bit only when `inter == 1`; on the progressive path it returns the literal 1 it loaded for the comparison, forever. That is the CSR.FIELD trap, in the game rather than the library, and one NOP at `001A0DD4` on the only return-value loop in the image ends it. Shipped as four words; the full working, the VRAM diff and the register decode are in [the devlog](devlog-SLUS-21207-dragon-quest-viii-no-interlacing.md).


**Radiata Stories has a real progressive mode, the game says so on screen, and the one-line autoboot works - drive the flag, never the branch.** `SLUS-21262`, undub disc, ELF CRC **`47B9B2FD`**; the bundle has no pnach for this serial at any CRC. The `fieldrender-gaps.tsv` row that measures it `640x448 -> 1280x447` is a 480i measurement, and the 480p list is the one that is right: the game's own Screen options page prints *"If you have a progressive scan television, press the Triangle button and the Cross button at the same time to switch to progressive scan mode."* So there is nothing to deinterlace and nothing to reverse-engineer - only a convenience to add.

The mode selector reads a byte flag at **`0019E9C4`**, in `.bss`, and calls `sceGsResetGraph(0, 0, 0x50, 1)` when it is set against `(0, 1, 2, 1)` when it is clear. NOPping the branch that tests it (`00124A7C: 10400008 -> 00000000`) reaches `SDTV 480p` and renders **half height** - `1280x448` at 2x where stock is `1280x896` - because forcing the call skips whatever else the real path does to the buffers. Setting the flag reaches the same mode at the full `1280x896`, pixel-for-pixel the stock picture with the interlace gone. Drive the setting, not the branch.

**Shipped** as a one-line `[No-Interlacing]` group, `patch=1,EE,0019E9C4,byte,00000001`. `place=1` is deliberate: the flag is in `.bss` and two further sites clear it - the video object's reset at `001244F8` and a `set_progressive(obj, flag)` twin at `00124150` that an earlier "exactly one accessor" note here missed, because both reach the byte through an object pointer rather than an absolute base. The cost is that it also overrides the in-game Triangle+Cross toggle, which the description says. Full working in [the devlog](devlog-SLUS-21262-radiata-stories-no-interlacing.md).

**3D gameplay is still unverified**, and that is the one thing left to check. It holds from the first frame through the settings screens and into the opening cinematic - but so did Dragon Quest VIII, which then blanked the moment it left them. Radiata cannot be driven past its first-boot Screen menu with `PostMessage` key input and its Triangle+Cross toggle does not register that way either, so the check needs a human on the pad.
...
**Mortal Kombat: Shaolin Monks does not reproduce its own field-render row, and that is worth someone else checking.** `SLUS-21087_455DD546` - the disc here hashes to the CRC the bundle already covers, so its widescreen file loads. This game is listed above under *Measured field renderers* on the strength of a `640x448 -> 1280x447` row in [`data/fieldrender-gaps.tsv`](data/fieldrender-gaps.tsv), and it is the one entry in this document with a live complaint attached. **It did not reproduce.** Like Dragon Quest VIII and Oni it has **no `64420008` anywhere in the image**, so there is no libgraph half-offset helper to fire; `$gp = 005B9FF0`, the `SetGsCrt` stub is at `0047FA60`, and the game's own GS display writes are the 22 `lui 0x1200` sites, spread across `0020606C`, `002315F0`, `0024A918`, `0027477C` and `003832DC` rather than gathered in a libgraph block.

Measured in Goro's Lair at 2x: `1280x896`, the vertical doubles. Eight screen-resolution frames in gameplay all score best at shift **0** with comb energy flat at 1.45; eight more on the **pause menu** - the screen the complaint actually names, since the community code is described as fixing 3D but not menus or HUD - are **byte-identical**, MSE 0.00, comb flat at 2.40. And that is not PCSX2 hiding it: the whole pause-menu burst was repeated with `disable_interlace_offset = false` instead of the default `true`, and came back byte-identical again, MSE 0.00, comb 2.42.

So on PCSX2 2.8.1, this build, these two scenes, there is nothing alternating. That is not the same as saying the survey is wrong - one level and one menu is not the whole game, and the arenas, the two-player modes and the front-end menus are all untested. But the row should not be treated as established either. Before anyone budgets TIER-A work here, reproduce the artefact first and record *which screen* it is on.
...
```
60 FPS off, interval 2   draw env OFY  29184 / 29184   delta 0
60 FPS on,  interval 1   draw env OFY  29192 / 29184   delta 8   = half a scanline in 12.4
```

Correct on a CRT, a bob in an emulator. The offset helper at `00105AC0` has three call sites; the one that fires is inside the present function, on the branch taken only while the mode global `002EE4CC` is 1 — a state the game enters around movie playback and the title screen. **That gate says where the offset is written, not where it is seen, and reading it as "only the movie-backed screens" was wrong.** The helper writes `XYOFFSET` into whichever of the two draw environments the buffer counter selects, nothing ever writes it back, and the present function alternates the two environments every frame: one pass through any mode-1 screen leaves the whole session hopping half a scanline at 30 Hz. The game boots through the attract movie, so gameplay, the pause menu and the status screen all inherit it — which is how it was finally reported, as a menu strobe. The fix is one word in the present function, `001312FC: lh a3, -0x754c(gp)` — `addiu a3, zero, 0`, handing the helper field 0; see [that patch's devlog](../60fps/devlog-SLUS-20893-way-of-the-samurai-2-60fps.md#fifth-pass-the-offset-sticks-and-it-is-the-menu-strobe). Of the other two call sites, one is gated on `002EE4A8`, which read 0 in every state sampled; the other sits in the vsync handler at `00164530`, which reads `CSR` bit 13 directly and drives the movie player's own environments at `00309440`/`00309530` off the interrupt — it runs at 60 Hz whatever the vsync interval is, so it is stock behaviour and no frame-rate patch reaches it. So when new shimmer is reported after a frame-rate patch, diff the two draw environments before blaming the emulator's deinterlacer — and diff them on the screen that is complained about, not on the screen where the code that writes them runs.
...
**Champions of Norrath NTSC-U is already solved, and checking the bundle for the wrong serial is how you miss it.** The retail NTSC-U release is `SLUS-20565`, not the `SLUS-207xx` a title-order guess produces, and PCSX2 2.8.1 ships `SLUS-20565_90E66BC5.pnach` carrying a working `[No-Interlacing]` group by Switz0018 that `EnableNoInterlacingPatches` turns on by default. An investigation that grepped `patches.zip` for a guessed serial found nothing, disassembled the whole engine, and only noticed the shipped patch when the emulator log said `Enabled patch: No-Interlacing` — after spending an hour on a "mystery writer" that was the pnach's own `place=1` lines reverting every experiment within two milliseconds. Read the serial off the disc (`SYSTEM.CNF`, or PINE's game-id) before anything else, and when live writes will not stick, check the log for a loaded pnach before suspecting the game.

The mechanism is worth recording anyway, because the ELF keeps a full symbol table (`.symtab`, 21415 symbols) and the globals name themselves. `engineFrameStart__FUib` at `00191200` stores a field index into `draw+8`: `fullFrameMode` non-zero writes 0, otherwise it writes the `vblankField` byte the vblank handler samples from `CSR` bit 13. `frameStartPacket__FUibi` turns that index into `engineYOffset` — `movz t0, a3, a2` at `0018FB80` picks `0x7208` for field 1 and `0x7200` for field 0 — and that word is the `OFY` half of the `XYOFFSET` the frame is drawn through. Measured on `90E66BC5`: patch off, `OFY` alternates `0x7200`/`0x7208` every frame in menus, the intro and gameplay alike; patch on, it is pinned at `0x7200` in all three. Eight in 12.4 fixed point, the same half scanline as everywhere else in this document.

**A shipped patch can work by accident, and this one does.** That group's first line is `patch=1,EE,001912CC,extended,0000102D`, annotated "Don't load the vblankField byte". The leading `0` of an `extended` address selects an **8-bit** write, so only `0x2D` lands: the instruction stays `lbu v0, imm(gp)` and goes from `9382CA4C` to `9382CA2D`, moving its displacement from `vblankField` at `0048443C` to `0048441D` — byte 1 of `engineDisable30fps`. The intended `move v0, zero` needed `201912CC`, which is the prefix the group's other three lines use. It works only because `engineDisable30fps` is a `u32` that never holds anything but 0 or 1, so that byte reads 0 forever; the effect is right and the reason is luck. Read `extended` addresses as opcodes, not as decoration, and verify a patched word by reading it back rather than by trusting the data column.

**A No-Interlacing group does not always want `gsinterlacemode=1`.** The 15% of shipped groups that omit it are not all oversights. This game programs `SMODE2 = 3` (`INT=1`, `FFMD=1`) with `DISPLAY` at `DW/MAGH = 640`, `DH = 448`, `MAGV = 1` — a full-height frame the CRTC reads every line of — and PCSX2 then has nothing to deinterlace. Cycling the mode on a static scene gives byte-identical PNGs for Automatic, Off and both Bob settings, and only the two Weave settings differ. So forcing Off would change no pixels, and the group is right to leave the user's setting alone. Where the buffer is full height and `FFMD = 1`, decide the key by measurement instead of by convention.
