...
The mode selector is at `00124A70` (`$gp = 001862F0`): it reads a byte flag at **`0019E9C4`** and calls `sceGsResetGraph(mode=0, inter=0, omode=0x50, ffmd=1)` when set, against `inter=1, omode=2` when clear. A twin at `00124150` takes the same choice as an argument. **`0019E9C4` has exactly one accessor in the boot ELF** - the `lbu` at `00124A78` - so whatever the menu handler writes it from lives in an overlay, not in the 518 KB boot image.

Two ways to force it, and only one is correct:

- **NOP the branch** (`00124A7C: 10400008 -> 00000000`) so the progressive arm always runs. PCSX2 duly logs `Mode Changed to SDTV 480p` - and renders a **half-height picture**, `1280x448` at 2x where stock is `1280x896`. Forcing the `sceGsResetGraph` call skips whatever else the real path does to the buffers. Wrong lever.
- **Set the flag** (`patch=1,EE,0019E9C4,byte,00000001`). Same `SDTV 480p`, and the render is the full `1280x896`, pixel-for-pixel the stock picture with the interlace gone. One line, and it is the game's own code doing the work.
