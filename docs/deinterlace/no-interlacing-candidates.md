...
**Then it goes black.** Two cold boots driven by the identical key sequence off the first-boot settings screen: stock reaches a picture with frame means of 81-117, the patched build stays at mean **0.0** across six captures over 40 seconds - with the log still reading SDTV 480p and the config struct still reading mode 2, so it is neither the navigation nor a mode switch. The cause is almost certainly that the game does not leave its CRTC to libgraph: of the 25 `lui 0x1200` sites in the image, **four sit outside the libgraph block** - `0014577C`, `00145910`, `00160540` and `00160C50` - and that last one is next door to the second `sceGsResetGraph` call site at `00160C48` and the 448-line constants at `00160C6C`. Those writes are built for interlaced NTSC and reprogram the display out from under the 480p setup. A complete patch has to bring them along. Left unshipped, and worth picking up: the game has no pnach of any kind and the hard half is already done.
